...
import { FaArrowLeft, FaCalendar, FaMapMarkerAlt, FaShare } from 'react-icons/fa';
...
export default function PlacePage({ article }: PlacePageProps) {
  ...
  const handleShare = async () => {
    const shareUrl = `${window.location.origin}/place/${article.id}`;
  ...
  return (
    <Layout title={`${article.title} - PattiBytes Places`} description={article.preview}>
      <article className={styles.article}>
        ...
        <div className={styles.articleHeader}>
          ...
          <div className={styles.meta}>
            <div className={styles.metaItem}>
              <FaMapMarkerAlt />
              <span>{article.location || 'Patti, Punjab'}</span>
            </div>
            <div className={styles.metaItem}>
              <FaCalendar />
        ...
        <div className={styles.articleFooter}>
          <p>Explore more places in Patti</p>
          <Link href="/places" className={styles.backToNews}>
            ← View All Places
          </Link>
        </div>
      </article>
...
export const getStaticPaths: GetStaticPaths = async () => {
  try {
    ...
    const res = await fetch(`${ORIGIN}/places/index.json`, {
      next: { revalidate: 60 }
    });

    if (!res.ok) throw new Error('Failed to fetch places index');
    ...
    console.error('Error generating static paths for places:', error);
    return {
      paths: [],
      fallback: true
    };
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  try {
    ...
    const ORIGIN = process.env.NEXT_PUBLIC_SITE_URL || 'https://www.pattibytes.com';
    const res = await fetch(`${ORIGIN}/places/${params.id}.json`, {
      next: { revalidate: 60 }
    });
