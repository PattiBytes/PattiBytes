...
export default async function handler(_req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    // Try app-next/_news (monorepo apps) then repo root /_news
    const appDir = path.join(baseDir, '_news');
    const rootDir = path.join(baseDir, '..', '_news');
    ...
    console.error('API /api/cms/news error:', err);
    res.status(200).json([]);
  }
}
