...
export function validateUsername(username: string): { valid: boolean; error?: string } {
  ...
  const inappropriate = ['fuck', 'shit', 'damn', 'hate', 'kill', 'die', 'sex'];
  if (inappropriate.some(word => trimmed.includes(word))) {
...
export async function checkUsernameAvailable(username: string): Promise<boolean> {
  ...
  if (!isFirebaseConfigured()) {
    ...
    throw new Error('Service unavailable. Please check your configuration.');
  }
...
export async function claimUsername(
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (usernameSnapshot.exists()) {
        ...
        // Username already belongs to this user, continue to update profile
      }
      ...
      if (isNewUser) {
        ...
        // Don't set isOnline during initial setup to avoid conflicts
      }
  ...
  // All retries failed
  console.error('All username claim attempts failed');
...
export async function updateUserProfile(
  ...
  try {
    ...
    delete profileUpdates.username; // Use claimUsername to change username
    delete profileUpdates.stats; // Stats are managed separately
...
export function getUsernameSuggestions(baseUsername: string, count: number = 3): string[] {
  ...
  if (!base) {
    // If no valid characters, return generic suggestions
    return [
  ...
  // Add prefixes/suffixes if we need more suggestions
...
// Clear username cache (for testing/debugging)
export function clearUsernameCache(): void {
  usernameCache.clear();
}

// Get cached username status (for debugging)
...
// Check if username exists (alias for checkUsernameAvailable)
export async function usernameExists(username: string): Promise<boolean> {
  const available = await checkUsernameAvailable(username);
  return !available;
}

// Get all reserved usernames (for documentation/UI)
