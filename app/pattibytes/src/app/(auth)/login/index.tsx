...
/**
 * Resolves email from either a direct email address or a @username.
 * Uses a SECURITY DEFINER RPC to bypass RLS for username lookup.
 */
...
export default function Login() {
  ...
  async function handleApple() {
    ...
    try {
      ...
      // ✅ Correct cancel code is ERR_REQUEST_CANCELED (not ERR_CANCELED or 1001)
      if (
  ...
  // Lock label: only change after user clears the field entirely
  const inputLabel =
    identifier.includes('@')
      ? 'Email'
      : identifier.length > 0
  ...
  const idBorderColor = idBorder.interpolate({
    inputRange: [0, 1], outputRange: [COLORS.border, COLORS.primary],
  })
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Apple Sign In */}
          {appleAvail && (
            // ✅ Use style pointerEvents instead of prop (deprecated in RN 0.74+)
            <View style={[S.appleWrap, { pointerEvents: busy ? 'none' : 'auto' as any },
              busy && { opacity: 0.55 }]}>
              {aLoading ? (
        ...
        <Text style={S.footer}>
          ...
          <Text style={S.footerLink} onPress={() => router.push('/legal/terms-of-service' as any)}>
            Terms
          </Text>{' '}
          and{' '}
          <Text style={S.footerLink} onPress={() => router.push('/legal/privacy-policy' as any)}>
            Privacy Policy
          </Text>.{'\n'}
          <Text
            style={{ color: '#9CA3AF' }}
...
const S = StyleSheet.create({
  ...
  logoFallback: { width: 80, height: 80, borderRadius: 40, backgroundColor: '#FFF3EE',
  ...
  inputWrap:{ borderWidth: 1.5, borderRadius: 12, backgroundColor: COLORS.backgroundLight, marginBottom: 13 },
  ...
  whyRow: { marginTop: 14, paddingTop: 12, borderTopWidth: 1, borderTopColor: COLORS.border },
  whyText: { fontSize: 11, color: COLORS.textMuted, textAlign: 'center', lineHeight: 16 },
