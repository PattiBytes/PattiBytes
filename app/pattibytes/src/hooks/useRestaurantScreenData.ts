// src/hooks/useRestaurantScreenData.ts
...
// ── Cache TTLs (ms) ───────────────────────────────────────────────────────────
const TTL_MAIN        = 5  * 60 * 1000  // 5 min  — merchant + menu + reviews
const TTL_TRENDING    = 10 * 60 * 1000  // 10 min — order-based trending
const TTL_OFFERS      = 5  * 60 * 1000  // 5 min  — offer badges
const TTL_BXGY        = 5  * 60 * 1000  // 5 min  — auto-apply BXGY promos
const TTL_RECOMMENDED = 15 * 60 * 1000  // 15 min — similar restaurants

// ── Cache key factory ─────────────────────────────────────────────────────────
const ck = (id: string, suffix: string) => `rest_${id}_${suffix}`

// ── Types ─────────────────────────────────────────────────────────────────────
export type MerchantRow = {
  id: string; user_id: string; business_name: string; cuisine_types?: any
  ...
  min_order_amount?: number | null; estimated_prep_time?: number | null
  opening_time?: string | null   // HH:MM:SS
  closing_time?: string | null   // HH:MM:SS  — NO opening_hours column
  address?: string | null; city?: string | null; state?: string | null
  postal_code?: string | null
  is_active?: boolean | null; is_verified?: boolean | null; is_featured?: boolean | null
}

export type AppSettingsRow    = { id: string; app_name?: string | null; show_menu_images?: boolean | null }
export type MenuItemRow       = {
  id: string; merchant_id: string; name: string; description?: string | null
  price: number; discount_percentage?: number | null; image_url?: string | null
  category?: string | null; category_id?: string | null
  is_available: boolean; is_veg: boolean; preparation_time?: number | null
  dish_timing?: string | null
}
export type ReviewRow         = { id: string; order_id?: string | null; customer_id: string; merchant_id: string; rating: number | null; comment?: string | null; created_at: string; customer_name?: string }
export type NotificationPrefs = { promos?: boolean; system?: boolean; order_updates?: boolean; review_updates?: boolean; [k: string]: any }
export type OfferBadge        = { label: string; sub_label?: string; promo_code?: string }
export type TrendingItem      = { key: string; menu_item_id?: string | null; name: string; image_url?: string | null; price?: number | null; is_veg?: boolean | null; discount_percentage?: number | null; total_qty: number }
export type RecommendedMerchant = { id: string; business_name: string; logo_url?: string | null; cuisine_types?: string[]; average_rating?: number | null; estimated_prep_time?: number | null; latitude?: number | null; longitude?: number | null; distance_km?: number | null; is_open_now?: boolean }
export type BxgyTarget        = { id: string; promo_code_id: string; side: 'buy' | 'get'; menu_item_id: string | null; category_id: string | null }
export type AutoApplyBxgyPromo = { promo: any; buy_targets: BxgyTarget[]; get_targets: BxgyTarget[] }
type OrderedItemMini          = { id?: string; menu_item_id?: string; name?: string; quantity?: number; price?: number; is_veg?: boolean; image_url?: string | null; discount_percentage?: number }
type CachedMain               = { merchant: MerchantRow; app_settings: AppSettingsRow | null; menu_items: MenuItemRow[]; reviews: ReviewRow[] }
...
function isPromoActiveNow(p: any, now: Date): boolean {
  if (p?.is_active === false) return false
  if (p?.valid_from) { const vf = new Date(p.valid_from); if (!Number.isNaN(vf.getTime()) && now < vf) return false }
  if (p?.valid_until) { const vu = new Date(p.valid_until); if (!Number.isNaN(vu.getTime()) && now > vu) return false }
  return true
}
...
function getAuthId(ctx: any): string | null {
  const id = ctx?.user?.id ?? ctx?.profile?.id ?? ctx?.authUser?.id ?? ctx?.id ?? null
...
export function useRestaurantScreenData(merchantId: string) {
  ...
  const userId  = useMemo(() => getAuthId(authCtx), [authCtx])
...
const openNow = useMemo(() => {
 ...
 return isOpenNow(merchant.opening_time ?? null, merchant.closing_time ?? null)
  ...
  const closedMessage = useMemo(() => {
    ...
    const fmt = (t: string | null | undefined): string | undefined => {
      if (!t) return undefined
      const [hh, mm] = String(t).split(':')
      const h = Number(hh)
      return `${h % 12 || 12}:${mm ?? '00'} ${h >= 12 ? 'PM' : 'AM'}`
    }
    const open = merchant.opening_time, close = merchant.closing_time
  ...
  // ── Review helpers ────────────────────────────────────────────────────────
  const enrichReviewNames = useCallback(async (base: ReviewRow[]): Promise<ReviewRow[]> => {
  ...
  const loadReviewItemsForVisible = useCallback(async (list: ReviewRow[]) => {
    ...
    for (const p of pairs) out[p.reviewId] = itemsByOrderId.get(String(p.orderId!)) ?? []
    setReviewItemsByReviewId(out)
  }, [])


  // ── loadUserSpecific (never cached — changes per user action) ─────────────
  const loadUserSpecific = useCallback(async () => {
    if (!userId || !UUID_REGEX.test(merchantId)) return
    try {
      ...
      const oid = (delivered.data as any)?.[0]?.id ?? null
      ...
      console.warn('[loadUserSpecific]', e.message)
      setIsFav(false); setNotificationPrefs(null)
      setHasDeliveredOrder(false); setDeliveredOrderId(null); setAlreadyReviewed(false)
    }
  }, [merchantId, userId])
  ...
  const loadTrendingFromOrdersItems = useCallback(async () => {
    ...
    // Cache hit → instant
    const cached = appCache.get<TrendingItem[]>(ck(merchantId, 'trending'))
    if (cached) { setTrending(cached); return }

    setTrendingLoading(true)
    try {
      const sinceIso = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()
      const { data, error } = await supabase
        .from('orders').select('id,status,created_at,items')
        .eq('merchant_id', merchantId).gte('created_at', sinceIso).neq('status', 'cancelled').limit(250)
      if (error) throw error

      const qtyByKey = new Map<string, { total_qty: number; sample?: OrderedItemMini }>()
      for (const o of (data ?? [])) {
        const items = Array.isArray((o as any).items) ? (o as any).items as OrderedItemMini[] : []
        for (const it of items) {
          const menuItemId = it.menu_item_id ? String(it.menu_item_id) : undefined
          const fallbackId = it.id ? String(it.id) : undefined
          ...
          qtyByKey.set(key, { total_qty: (prev?.total_qty ?? 0) + q, sample: prev?.sample ?? it })
        }
      }

      const top = Array.from(qtyByKey.entries()).sort((a, b) => b[1].total_qty - a[1].total_qty).slice(0, 10)
      const menuIds = top.map(([k]) => k).filter(k => UUID_REGEX.test(k))
      const menuMap = new Map<string, any>()
      if (menuIds.length) {
        const { data: mi } = await supabase.from('menu_items').select('id,name,price,image_url,is_veg,discount_percentage').in('id', menuIds).limit(200)
        ;(mi ?? []).forEach((r: any) => menuMap.set(String(r.id), r))
      }

      const result: TrendingItem[] = top.map(([key, v]) => {
      ...
      appCache.set(ck(merchantId, 'trending'), result, TTL_TRENDING)
      setTrending(result)
    } catch {
      setTrending([])
    } finally {
  ...
  const loadOfferBadges = useCallback(async () => {
    ...
    const cached = appCache.get<Record<string, OfferBadge | null>>(ck(merchantId, 'offers'))
    if (cached) { setOfferByMenuItemId(cached); return }
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .eq('is_active', true).eq('scope', 'merchant').eq('merchant_id', merchantId)
      ...
      for (const p of promos) {
        ...
        if (!label) continue
        const badge: OfferBadge = { label, sub_label: p.auto_apply ? 'Auto offer' : 'Offer', promo_code: String(p.code ?? '') }
        ...
        if (!menuIds.length && !catIds.length) { if (!out['merchant:all']) out['merchant:all'] = badge }
        for (const id of menuIds)  if (!out[id])           out[id]           = badge
        for (const cid of catIds)  if (!out[`cat:${cid}`]) out[`cat:${cid}`] = badge
      }

      appCache.set(ck(merchantId, 'offers'), out, TTL_OFFERS)
  ...
  // ── Auto-apply BXGY promos ────────────────────────────────────────────────
  const loadAutoApplyBxgyPromos = useCallback(async () => {
    if (!UUID_REGEX.test(merchantId)) return

    const cached = appCache.get<AutoApplyBxgyPromo[]>(ck(merchantId, 'bxgy'))
    if (cached) { setAutoApplyBxgyPromos(cached); return }
    ...
    try {
      ...
      const { data: promoData, error: promoErr } = await supabase
        ...
        .eq('is_active', true).eq('scope', 'merchant').eq('merchant_id', merchantId)
        .eq('deal_type', 'bxgy').eq('auto_apply', true)
      ...
      const { data: targetData, error: targErr } = await supabase
        .from('promo_bxgy_targets').select('id,promo_code_id,side,menu_item_id,category_id').in('promo_code_id', ids)
      if (targErr) throw targErr

      const enriched: AutoApplyBxgyPromo[] = activePromos.map((promo: any) => ({
        promo,
        buy_targets: (targetData ?? []).filter((t: any) => t.promo_code_id === promo.id && t.side === 'buy') as BxgyTarget[],
        get_targets: (targetData ?? []).filter((t: any) => t.promo_code_id === promo.id && t.side === 'get') as BxgyTarget[],
      }))

      appCache.set(ck(merchantId, 'bxgy'), enriched, TTL_BXGY)
  ...
  const loadRecommendedRestaurants = useCallback(async (m: MerchantRow | null) => {
    ...
    const cacheKey = ck(merchantId, `recommended_${m.city ?? 'any'}`)
    const cached = appCache.get<RecommendedMerchant[]>(cacheKey)
    if (cached) { setRecommended(cached); return }
    ...
    try {
      ...
      let q = supabase
        ...
        .eq('is_active', true).neq('id', m.id).limit(40)
      if (city)  q = (q as any).eq('city',  city)
      if (state) q = (q as any).eq('state', state)

      const { data, error } = await q
      if (error) throw error

      const mLat = m.latitude  != null ? Number(m.latitude)  : null
      const mLon = m.longitude != null ? Number(m.longitude) : null

      const pool: RecommendedMerchant[] = (data ?? []).map((r: any) => {
        const rLat = r.latitude  != null ? Number(r.latitude)  : null
        const rLon = r.longitude != null ? Number(r.longitude) : null
        const dist = Number.isFinite(rLat!) && Number.isFinite(rLon!) && Number.isFinite(mLat!) && Number.isFinite(mLon!)
          ? haversineKm(mLat!, mLon!, rLat!, rLon!) : null
        return {
          id: String(r.id), business_name: String(r.business_name ?? 'Restaurant'),
          logo_url: r.logo_url ?? null,
          cuisine_types: normalizeCuisineTypes(r.cuisine_types),
          average_rating: r.average_rating != null ? Number(r.average_rating) : null,
          estimated_prep_time: r.estimated_prep_time != null ? Number(r.estimated_prep_time) : null,
          latitude: rLat, longitude: rLon, distance_km: dist,
      ...
      appCache.set(cacheKey, result, TTL_RECOMMENDED)
  ...
  // ── Main load (checks cache → instant render, then user-specific in bg) ───
  const loadAll = useCallback(async () => {
    if (!UUID_REGEX.test(merchantId)) return

    // ── Cache HIT: render instantly, refresh user-specific in background ──
    const cached = appCache.get<CachedMain>(ck(merchantId, 'main'))
    if (cached) {
      setMerchant(cached.merchant)
      setAppSettings(cached.app_settings)
      setMenuItems(cached.menu_items)
      setReviews(cached.reviews)
      loadReviewItemsForVisible(cached.reviews)
      setLoading(false)
      // Still fetch per-user data in the background (isFav, hasDeliveredOrder, etc.)
      if (userId) loadUserSpecific()
      return
    }

    // ── Cache MISS: full network fetch ────────────────────────────────────
    setLoading(true)
    try {
  ...
  .from('merchants')
  .select(
    'id,user_id,business_name,cuisine_types,description,logo_url,banner_url,' +
    'phone,email,latitude,longitude,is_active,is_verified,average_rating,' +
    'total_reviews,min_order_amount,estimated_prep_time,is_featured,' +
  ...
  .maybeSingle(),
        ...
        supabase
          ...
          .eq('merchant_id', merchantId).order('category', { ascending: true }).order('name', { ascending: true }),
        supabase
          .from('reviews')
      ...
      // Cache the non-user-specific data
      if (merch) {
        appCache.set(ck(merchantId, 'main'), {
          merchant:     merch,
          app_settings: (settingsRes.data as any) ?? null,
          menu_items:   (menuRes.data as any) ?? [],
          reviews:      enriched,
        }, TTL_MAIN)
      }

      // User-specific data — fetched in background, not cached
      if (userId) {
        loadUserSpecific()
      } else {
        setIsFav(false); setNotificationPrefs(null)
  ...
  }, [enrichReviewNames, loadReviewItemsForVisible, loadUserSpecific, merchantId, userId])
  ...
  useEffect(() => { loadRecommendedRestaurants(merchant) }, [merchant, loadRecommendedRestaurants])
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reviews', filter: `merchant_id=eq.${merchantId}` }, () => {
        ...
        reviewsReloadThrottle.current = setTimeout(async () => {
          ...
          // Bust main cache so reviews are fresh
          appCache.delete(ck(merchantId, 'main'))
          await loadAll()
        }, 900)
      })
      .subscribe()
    return () => {
      ...
      if (reviewsReloadThrottle.current) { clearTimeout(reviewsReloadThrottle.current); reviewsReloadThrottle.current = null }
    }
  }, [loadAll, merchantId])


  // ── Refresh (pull-to-refresh — busts all caches) ──────────────────────────
  const refresh = useCallback(async () => {
    // Bust every per-merchant cache key
    appCache.delete(ck(merchantId, 'main'))
    appCache.delete(ck(merchantId, 'trending'))
    appCache.delete(ck(merchantId, 'offers'))
    appCache.delete(ck(merchantId, 'bxgy'))
    appCache.delete(ck(merchantId, `recommended_${merchant?.city ?? 'any'}`))
    ...
    setRefreshing(false)
  }, [loadAll, loadOfferBadges, loadTrendingFromOrdersItems, loadAutoApplyBxgyPromos, loadRecommendedRestaurants, merchant, merchantId])
  ...
  return {
    ...
    recommended, recommendedLoading, offerByMenuItemId,
    reviews, reviewItemsByReviewId, isFav, openNow, closedMessage,
