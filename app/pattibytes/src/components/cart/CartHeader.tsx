...
const META: Record<OrderType, { label: string; emoji: string; color: string; bg: string }> = {
  restaurant: { label: 'Restaurant Order', emoji: '🍽️', color: '#B45309', bg: '#FEF3C7' },
  store:      { label: 'PBExpress Store',  emoji: '📦', color: '#5B21B6', bg: '#EDE9FE' },
  custom:     { label: 'Custom Order',     emoji: '✏️', color: '#065F46', bg: '#ECFDF5' },
}
  ...
  return (
    <View style={S.wrap}>
      ...
      <View style={[S.badge, { backgroundColor: m.bg }]}>
        ...
        {isFreeDelivery && (
          <View style={S.freeTag}>
            <Text style={{ color: '#fff', fontSize: 9, fontWeight: '800' }}>FREE DELIVERY</Text>
          </View>
        )}
      </View>
      ...
      <View style={S.row}>
        ...
        {/* Add more items shortcut (restaurant only) */}
        {isRestaurant && merchantId && (
          <TouchableOpacity
            onPress={() => router.push(`/(customer)/restaurant/${merchantId}` as any)}
            style={S.addBtn}
          >
