...
ttsToggleBtn.addEventListener("click", () => {
  ...
  /* ---------------------------
   TTS helpers: smooth scroll + offsets + efficient highlight
   Paste/replace inside your news.js (where TTS functions live)
----------------------------*/
...
function throttle(fn, wait = 120) {
  ...
  return function (...args) {
    const now = Date.now();
    const remaining = Math.max(0, wait - (now - last));
    clearTimeout(timer);
    if (remaining === 0) {
      last = now;
      fn.apply(this, args);
...
// Smooth scroll the modal scroll container so the element appears near center
function smoothScrollToElement(el, container) {
  try {
    container = container || el.closest(".modal-content") || document.scrollingElement;
    const elRect = el.getBoundingClientRect();
    const containerRect = container.getBoundingClientRect();

    // compute current scrollTop + distance to element center
    const containerScrollTop = container.scrollTop;
    const offsetTop = containerScrollTop + (elRect.top - containerRect.top) - container.clientHeight / 2 + el.offsetHeight / 2;
...
const throttledScroll = throttle(smoothScrollToElement, 160);

/* prepareTextForReading: wraps words in spans AND builds cumulative offsets */
function prepareTextForReading() {
  ...
  // nodes to read (exclude meta/published)
  const nodes = Array.from(
    modalTextContainer.querySelectorAll("p, h1, h2, h3, h4, li")
  ).filter((el) => el.textContent.trim() !== "" && !el.closest(".modal-meta") && !el.closest(".published"));

  const readContainer = document.createElement("div");
  readContainer.className = "tts-read-container";

  // We'll create spans and also compute cumulative char offsets for quick binary search
  const wordSpans = [];
  let cumulative = 0;
  const offsets = []; // will hold {start, end} for each span
  ...
  nodes.forEach((el) => {
    ...
    words.forEach((w, i) => {
      ...
      offsets.push({ start: cumulative, end: cumulative + len - 1, span });
      cumulative += len;
      wordSpans.push(span);
    });
    readContainer.appendChild(newEl);
  ...
  // place meta back + readContainer
  const meta = modalTextContainer.querySelector(".modal-meta");
  modalTextContainer.innerHTML = "";
  if (meta) modalTextContainer.appendChild(meta);
  modalTextContainer.appendChild(readContainer);

  // store offsets for later fast lookup
  modalTextContainer._ttsOffsets = offsets;
  modalTextContainer._ttsTotalChars = cumulative;
}

/* buildQueue unchanged in concept, but ensure it uses the read-container text reliably */
function buildQueue() {
  const readContainer = modalTextContainer.querySelector(".tts-read-container");
  if (!readContainer) {
    queue = [];
  ...
  // chunk by paragraph items (keeps chunks reasonable)
...
/* highlightByCharIndex: find word span quickly using precomputed offsets (binary search) */
function highlightByCharIndex(charIndex) {
  const offsets = modalTextContainer._ttsOffsets || [];
  if (!offsets.length) return;

  // binary search for the offset that contains charIndex
  let lo = 0, hi = offsets.length - 1, mid, found = -1;
  while (lo <= hi) {
    mid = Math.floor((lo + hi) / 2);
    const o = offsets[mid];
    if (charIndex < o.start) {
      hi = mid - 1;
    } else if (charIndex > o.end) {
      lo = mid + 1;
    } else {
      found = mid;
      break;
  ...
  qa(".tts-highlight", modalTextContainer).forEach((el) => el.classList.remove("tts-highlight"));
  const span = offsets[found].span;
  span.classList.add("tts-highlight");

  // smooth scroll only occasionally (throttled) to avoid jank
  throttledScroll(span, modalTextContainer.closest(".modal-content") || document.scrollingElement);

  // update currentWord index
  const wordSpans = qa(".tts-word-span", modalTextContainer);
  currentWord = Math.min(wordSpans.length, found + 1);
}

/* speakNextChunk: attach onboundary => use charIndex mapping and fallback gracefully */
function speakNextChunk() {
  if (!window.speechSynthesis) {
    statusSpan.textContent = "TTS unsupported";
    return;
  ...
  // onboundary may fire with charIndex relative to this chunk; we need to map to absolute offsets:
  // compute chunkStartCharOffset: sum lengths of previous chunks in queue (approx)
  let chunkStart = 0;
  try {
    // compute by summing lengths of earlier queue items
    for (let i = 0; i < queuePos - 1; i++) {
      chunkStart += (queue[i] ? queue[i].length : 0);
    }
  } catch (e) {
    chunkStart = 0;
  }
  ...
  utter.onboundary = (ev) => {
    if (ev.name === "word") {
      // absoluteCharIndex = chunkStart + ev.charIndex
      const absIndex = Math.max(0, (chunkStart || 0) + (ev.charIndex || 0));
      highlightByCharIndex(absIndex);
    }
  };
  ...
  utter.onend = () => {
    ...
    if (progressEl) {
      const total = modalTextContainer._ttsOffsets ? modalTextContainer._ttsOffsets.length : (qa(".tts-word-span", modalTextContainer).length || 1);
      currentWord = Math.min(total, currentWord + (text.split(/\s+/).length));
      const pct = Math.round((currentWord / total) * 100);
      progressEl.textContent = ` ${pct}%`;
    }
