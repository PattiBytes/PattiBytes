...
// ✅ Import the standalone function, not the class instance
...
function SignupForm() {
  ...
  const [formData, setFormData]         = useState({
    fullName: '', email: '', phone: '', password: '', role: roleParam,
  });
  ...
  const notifyAdmins = async (userName: string, userRole: string, userId: string) => {
    try {
      ...
      if (!admins?.length) { console.log('No admins found to notify'); return; }

      // ✅ Use standalone sendNotification() — not notificationService.sendNotification()
      await Promise.all(
        admins.map(admin =>
          sendNotification(
            admin.id,
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 hover-lift">
          <div className="text-center mb-8">
            <div className="relative w-20 h-20 mx-auto mb-4">
              <Image src="/icon-192.png" alt="PattiBytes" fill sizes="80px" className="object-contain animate-scaleIn" priority />
            </div>
