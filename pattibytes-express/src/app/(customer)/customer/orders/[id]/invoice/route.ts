...
export async function GET(_: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  // Ensure this user is a merchant & get merchant_id
  ...
  // FIX: Supabase relation can be typed as array -> normalize
  ...
  const pdfBuffer: Buffer = await new Promise((resolve, reject) => {
    ...
    doc.fontSize(10)
      .fillColor('#555')
      .text(`Payment: ${String(order.payment_method || '').toUpperCase()} • ${String(order.payment_status || '').toUpperCase()}`);
  ...
  // FIX: return Uint8Array so TS accepts body
