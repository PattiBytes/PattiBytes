...
import { ShoppingCart, ArrowRight, Search, X, Loader2 } from 'lucide-react';
...
function ShopPageInner() {
  ...
  const handleAdd = useCallback((product: CustomProduct, qty: number) => {
    ...
    toast.success(`${product.name} added to cart!`, { position: 'bottom-center', autoClose: 1200 });
  }, [cart, addToCart]);
  ...
  const handleUpdateQty = useCallback((productId: string, delta: number) => {
    const newQty = Math.max(1, Math.min(20, (cartQtyMap[productId] ?? 0) + delta));
    updateQuantity(productId, newQty);
  }, [cartQtyMap, updateQuantity]);

  const cartCount = useMemo(() => cart?.items?.reduce((s, i) => s + i.quantity, 0) ?? 0, [cart?.items]);
  ...
  return (
    <AppShell title="Shop">
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-pink-50">
        ...
        {cartCount > 0 && (
          <div className="fixed bottom-4 left-4 right-4 z-50 animate-in slide-in-from-bottom duration-400">
            <button
              onClick={() => router.push('/customer/cart')}
            ...
            >
              ...
              <div className="flex items-center gap-1">
                <span>Rs.{cartTotal.toFixed(0)} - Go to Cart</span>
    ...
    <AppShell title="Shop">
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-orange-50 via-white to-pink-50">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold text-gray-500">Loading shop...</p>
        </div>
      </div>
    </AppShell>
  );
