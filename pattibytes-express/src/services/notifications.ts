...
export async function sendNotification(
  ...
  try {
    ...
    if (!jwt) {
      console.warn('[sendNotification] No session — skipping');
      return false;
    }
    ...
    if (!res.ok) {
      ...
      console.error('[sendNotification] API error:', err);
      return false;
    }
...
// ── NotificationService class (CRUD — unchanged) ──────────────────────────────
class NotificationService {
  ...
  async sendOrderNotification(orderId: string, status: string) {
    const { data: order, error } = await supabase
      .from('orders').select('id, order_number, customer_id, merchant_id')
    ...
    if (msgs[status]?.customer && order.customer_id) {
      await sendNotification(order.customer_id, title, msgs[status]!.customer!, 'order', notifData,
        { url: `/orders/${orderId}` });
    }
    if (msgs[status]?.merchant && order.merchant_id) {
      ...
      if (merchant?.user_id) {
        await sendNotification(merchant.user_id, title, msgs[status]!.merchant!, 'order', notifData,
